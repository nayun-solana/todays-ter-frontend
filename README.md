...
배포된 API 명세(`/v3/api-docs`)와 실제 호출로 대조한 결과입니다. (2026-08-13 기준)
...
| 마이 → 사주 리포트 다시보기                          | 실연동     | `GET /fortune-reports/me`                                |
| 고민 유형 수정                                       | 실연동     | `PUT /members/me/concerns` — 저장 후 홈·추천 캐시 무효화 |
| 약관 화면                                            | 실연동     | `GET /mypage/policies`                                   |
| 권한 안내 화면                                       | 실연동     | 서버가 아니라 브라우저 Permissions API 기준으로 표시     |
| PWA(설치·자산 캐시·새 버전 안내)                     | 실연동     | 빌드 설정                                                |
| 알림(목록·미읽음 개수·읽음 처리·설정)                | **미노출** | API·화면 모두 있으나 앱 내 진입점을 제거한 상태(#186)    |
| 소셜 로그인(Apple·Google)                            | **미제공** | BE 미구현(`/auth/*`에 경로 없음) — 로그인 화면에서 제거  |

FE가 호출하는 49개 경로는 모두 배포 명세에 존재합니다.

**목데이터로 동작하는 화면은 없습니다.** 개발용 MSW 모킹을 제거했고, 서버 응답이 없으면 화면을 임의의 값으로 채우지 않고 로딩·에러 상태를 그대로 드러냅니다. `미노출` 항목은 화면·API 코드를 그대로 두되 앱 안에서 들어가는 경로만 없앤 상태로, 다시 노출하려면 진입점만 되살리면 됩니다.
...
| 상태·통신   | TanStack Query 5, Zustand, Axios, Zod                          |
