...
/**
 * 이용약관 및 개인정보처리방침 전문(2026-08-07 시행).
 * 서버 `GET /mypage/policies`는 아직 초안이라 URL이 없어, 문서를 앱 안에 그대로 둔다.
 */
const DOCUMENT = `
# 오늘의터 서비스 이용약관
...
이 약관은 오늘의터(이하 "회사"라 함)이 제공하는 서비스의 이용과 관련하여 회사와 이용자 간의 권리, 의무 및 책임사항, 기타 필요한 사항을 규정함을 목적으로 합니다.
...
2. 회사는 다음 각 호에 해당하는 신청에 대해서는 승낙을 하지 않거나 사후에 이용계약을 해지할 수 있습니다. 회사는 이용신청 요건을 충족하는 모든 이용자의 신청을 승낙합니다.
...
1. 회사는 다음과 같은 서비스를 제공합니다. 사주 분석, 장소 큐레이션, 장소 탐색, 후기 기록
2. 회사는 상당한 이유가 있는 경우에 운영상, 기술상의 필요에 따라 제공하고 있는 서비스를 변경할 수 있습니다.
3. 회사는 이용자에게 서비스를 제공함에 있어 관련 법령, 약관, 운영정책 및 공지사항 등에서 정한 바에 따라 무료로 서비스를 제공합니다.
...
1. 회사는 회원이 서비스 이용 중 필요하다고 인정되는 다양한 정보를 공지사항이나 앱/웹 내 알림, 앱 푸시 알림 등의 방법으로 회원에게 제공할 수 있습니다.
2. 회원은 관련 법령에 따른 거래관련 정보 및 고객문의 등에 대한 답변 등을 제외하고는 언제든지 위 정보제공에 대하여 수신 거절을 할 수 있습니다.
3. 회사는 서비스의 운영과 관련하여 서비스 화면, 홈페이지, 전자우편 등에 광고를 게재할 수 있습니다. 광고가 게재된 전자우편을 수신한 회원은 수신 거절을 회사에게 할 수 있습니다.
...
1. 회원의 게시물이 관련 법령, 본 약관에 위반되거나 타인의 권리를 침해한다고 판단되는 경우, 회사는 관련 법령에 따라 해당 게시물에 대한 접근을 임시적으로 차단하거나 삭제할 수 있습니다.
2. 회사가 제1항에 따라 회원의 게시물을 삭제하거나 게시 중단 조치를 취하는 경우, 회사는 해당 조치의 사유를 회원에게 통지합니다. 다만, 긴급한 경우에는 사후에 통지할 수 있습니다.
...
3. 회사는 회원의 명시적인 동의 없이 회원의 게시물을 상업적으로 이용하지 않습니다.
4. 회원은 언제든지 서비스 내 관리기능을 통해 자신의 게시물에 대해 삭제, 수정, 비공개 등의 조치를 취할 수 있습니다.
...
1. 이 약관은 2026-08-07부터 시행합니다.
`;

// ponytail: 문서 한 장짜리라 마크다운 라이브러리 대신 줄 단위 렌더. 문서가 늘거나 링크·강조가
// 필요해지면 그때 파서를 붙인다.
function Line({ text }: { text: string }) {
  if (text.startsWith('# ')) {
    return <h2 className="typo-head-3 mt-6 text-gray-6 first:mt-0">{text.slice(2)}</h2>;
  }
  if (text.startsWith('## ')) {
    return <h3 className="typo-head-4 mt-5 text-gray-6">{text.slice(3)}</h3>;
  }
  // 원문 들여쓰기(4칸 = 한 단계)를 그대로 단계별 여백으로 옮긴다.
  const indent = text.length - text.trimStart().length;
  return (
    <p className="typo-sub-2 text-gray-5" style={{ paddingLeft: indent * 4 }}>
      {text.trimStart()}
    </p>
  );
}
...
export default function PrivatePage() {
  ...
  return (
    <div className="min-h-dvh w-full bg-gray-1">
      <PageHeader title="개인정보 및 약관" />

      <main className="flex flex-col gap-2 px-5 py-5 pb-[calc(2rem+env(safe-area-inset-bottom))]">
