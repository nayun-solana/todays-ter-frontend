          ...
          picks.map((pick) => {
            ...
            return (
              <button
                ...
                className="flex h-20 items-center justify-between rounded-btn bg-white px-4 py-3.5 text-left shadow-card"
              >
                <div className="flex items-center gap-3">
                  ...
                  <div className="flex flex-col justify-center gap-2">
                    <p className="flex items-center gap-1.5">
                      ...
                      <span className="text-sm leading-4 font-bold text-gray-6">{pick.name}</span>
                      <span aria-hidden="true" className="size-[2px] rounded-full bg-gray-3" />
                      <span className="text-[10px] leading-[14px] font-normal text-gray-4">
                        {pick.course}
                      </span>
                    </p>
                    {/* 설명만 오행 색을 쓴다 — 카드 배경은 흰색이다(탐색에 있던 오행색 배경과 다름). */}
                    <p className={cn('text-xs leading-4 font-bold', meta.text)}>
                      {pick.description}
                    </p>
                  </div>
                </div>
