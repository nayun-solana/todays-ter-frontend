...
| 알림(목록·미읽음 개수·읽음 처리·설정)                | **UI만** | `/notifications/*` 서버 미배포                 |
| 권한·약관 화면                                       | **UI만** | `/mypage/*` 서버 미배포                        |
| 마이 → 사주 리포트 다시보기                          | **UI만** | `GET /fortune-reports/me` 서버 미배포          |
| 고민 유형 수정                                       | **UI만** | 회원용 저장 API 없음(게스트 전용만 존재)       |

**목데이터로 동작하는 화면은 없습니다.** 개발용 MSW 모킹을 제거했고, 서버 응답이 없으면 화면을 임의의 값으로 채우지 않고 로딩·에러 상태를 그대로 드러냅니다. 위 `UI만` 항목은 화면과 API 호출까지 구현돼 있고 서버 배포만 남은 상태입니다.
